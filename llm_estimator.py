...
def estimate_hardware(model_params, tpot, ttft):
    ...
    # 2. 计算所需算力 (TOPS)
    alpha = 0.2
    ops_per_token = 2 * P * (1 + alpha)  # MAC ops
    tops_req = ops_per_token * B * L / 1e12 / (ttft / 1000)
...
def main():
    ...
    bandwidth_req, tops_req, sram_min_mb = estimate_hardware(model_params, args.tpot, args.ttft)
